const urls = require('../fixtures/final.json');
...
describe(`make screenshots`, () => {
    function createScreenshot(size, url, file, current_url) {
        ...
        cy.visit(current_url);
        cy.screenshot(`${url}${file}_${curr_name}`, {
            overwrite: true
        });
    }
    ...
    urls.forEach(url => {
        files.forEach(file => {
            ...
            for (let i = 0; i < sizes.length; i++) {
                ...
                screenshot_url = `cypress/screenshots/week${lesson}_spec.js/${url}${file}_${curr_name}.png`

                student_string += `students.push({name:'${url}${file}_${curr_name}',link:'${screenshot_url}'});`;
            }
        })
    })
